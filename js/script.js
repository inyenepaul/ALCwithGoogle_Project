;
(function (global) {
    var AddressBook = function (name, phone, email, image) {
        return new AddressBook.init(name, phone, email, image);
    };
    ...
    AddressBook.prototype = {
        ...
        addNewContact: function (name, phone, email, image) {
            this.data.push({
                name: name,
                phone: phone,
                email: email,
    ...
    AddressBook.init = function (name, phone, email, image) {
        var self = this;
        //set up the address book
        if (name || phone || email || image) {
            self.addNewContact(name || "", phone || "", email || "", image || "");
        }
...
form.addEventListener('submit', function () {
    if (!window.contactList) { //check if we already have a contact list
        window.contactList = $ab(form.person.value, form.phone.value, form.email.value, form.image.value);
    } else {
        //saves new values rather than deleting old ones as well
        contactList.addNewContact(form.person.value, form.phone.value, form.email.value, form.image.value);
    }
...
searchForm.addEventListener('submit', function () {
    ...
    if (results.length > 0) {
        ...
        for (var i = 0; i < results.length; i++) {
            document.getElementById('results').innerHTML += '<div class="contact-item">Name:' + results[i].name + '<br>Phone:' + results[i].phone + '<br>Email:' + results[i].email + ' + ' < br >Picture: ' + results[i].image + ' < /div><hr>';
        }
    } else {
